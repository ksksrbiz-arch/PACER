...
class PortfolioManager:
    ...
    async def add_from_candidate(
        ...
        *,
        ...
        entry = DomainPortfolio(
            ...
            current_valuation_usd=self._estimate_valuation(candidate),
        ...
        logger.info(
            f"Portfolio: recorded {domain!r} "
            f"(score={candidate.seo_score} strategy={monetization_strategy} "
            f"valuation={entry.current_valuation_usd})"
        )
        await APIResilience.log_compliance(
            "portfolio_entry_created",
            {
                ...
                "monetization_strategy": monetization_strategy,
                "redirect_target": redirect_target,
                "estimated_valuation_usd": entry.current_valuation_usd,
            },
        )
        return entry
    ...
    def compute_portfolio_summary(self, entries: list[DomainPortfolio]) -> dict:
        ...
        Returns a dict with:
          - total_domains
          - active / pending / expired / sold counts
          - total_valuation_usd
          - avg_seo_score
        ...
        total_valuation = sum(
            (e.current_valuation_usd or 0.0) for e in entries
        )
    ...
    def find_expiring_soon(
        ...
        for entry in entries:
            ...
            try:
                ...
                logger.warning(f"Invalid renewal_date for {entry.domain!r}: {entry.renewal_date!r}")
                continue
    ...
    def update_valuation(
        ...
        logger.info(
            f"Valuation updated for {entry.domain!r}: "
            f"${old:.2f} → ${new_valuation_usd:.2f}"
        )
        return entry
    ...
    def _estimate_valuation(candidate: DomainCandidate) -> float:
        """
        Rough valuation estimate based on SEO score.

        Formula: $100 × seo_score (linear proxy until Doma/Ahrefs data is available).
